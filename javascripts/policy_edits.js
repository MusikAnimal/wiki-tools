...
var contrib = require("../views/policy_edits/contrib.handlebars");
...
WT.formSubmit = function(e) {
  ...
  history.pushState({}, username + " - Policies and Guidelines Edit Counter from MusikAnimal", WT.path + "?" + this.params);
...
function startOver() {
  ...
  userData = {};
  history.pushState({}, "Policies and Guidelines Edit Counter from MusikAnimal", WT.path);
}
...
function insertContribs(resp) {
  // var policyContribs, guidelineContribs;

  // _.each(resp.contribs, function(contrib) {
  //   if(/policy/.test(contrib)) {
  //     policyContribs.push(contrib);
  //   }
  //   if(/guideline/.test(contrib)) {
  //     guidelineContribs.push(contrib);
  //   }
  // });
